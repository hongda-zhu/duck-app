describe('logic - retrieve user', () => {
    ...
    beforeEach(done => {
        ...
        call('POST', 'https://hongdazhu86.herokuapp.com/api/user', undefined, { name, surname, username: email, age, password }, result => {
            if (result.error) done(new Error(result.error))
            else {
                call('POST', 'https://hongdazhu86.herokuapp.com/api/auth', undefined, { username: email, password }, result => {
                    if (result.error) done(new Error(result.error))
                    else {
                        const { data } = result
                        id = data.id
